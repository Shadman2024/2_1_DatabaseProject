...
function ItemSaleForm() {
    ...
    const { itemName, itemDescription, itemPrice, itemImage, itemCategory, itemSubcategory } = item;
        ...
        if (name === 'itemCategory') {
            if (value === 'Add New Category...') {
                ...
                if (newCategory && !categories.includes(newCategory)) {
                    ...
                    setItem(prevItem => ({ ...prevItem, itemCategory: newCategory, itemSubcategory: '' }));
                ...
                setItem(prevItem => ({ ...prevItem, [name]: value, itemSubcategory: '' }));
            }
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch('http://localhost:5000/addItem/getCategory');
                if (!response.ok) throw new Error(`Network response was not ok: ${response.statusText}`);
                const data = await response.json();
                setCategories([...data.map(cat => cat.name), 'Add New Category...'].sort());
            } catch (error) {
    ...
    useEffect(() => {
        const fetchSubcategories = async () => {
            if (item.itemCategory && item.itemCategory !== 'Add New Category...') {
                try {
                    const response = await fetch(`http://localhost:5000/addItem/getSubcategory/${item.itemCategory}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const formData = { itemName, itemDescription, itemPrice, itemImage, itemCategory, itemSubcategory };
        try {
        ...
        <div className={styles.formContainer}>
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                ...
                <div className={styles.divName}>
                    <label htmlFor="itemName" className={styles.namelabel}>Item Name:</label>
                    <input type="text"  
                    name="itemName" 
                    value={item.itemName} 
                ...
                <div className={styles.divDescription}>
                    <label htmlFor="itemDescription" className={styles.descriptionlabel}>Item Description:</label>
                    <input  type="text" 
                ...
                <div className={styles.divPrice}>
                    <label htmlFor="itemPrice" className={styles.pricelabel}>Price:</label>
                    <input type="number" 
                     name="itemPrice" 
                ...
                <div className={styles.divImage}>
                    <label htmlFor="itemImage" className={styles.imagelabel}>Upload Image:</label>
                    <input type="text"  
                    name="itemImage" 
                ...
                <div className={styles.divCategory}>
                    <label htmlFor="itemCategory" className={styles.categorylabel}>Category:</label>
                ...
                {subcategories.length > 0 && (
                    <div className={styles.divsubcategory}>
                        <label htmlFor="itemSubcategory" className={styles.subcategorylabel}>Subcategory:</label>
