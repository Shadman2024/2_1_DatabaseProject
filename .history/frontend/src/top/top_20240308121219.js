  ...
  useEffect(() => {
    const fetchtopsellers = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/home/topsellers/${timeFrame2.toLowerCase()}`, {
          method: 'GET',
          headers: {
        ...
        <div className={styles.topitem2}>
          ...
          <div className={styles.contentcontainer}>
            {topsellers.map((topseller, index) => (
              <div key={index} className={styles.content}>
                <div className={styles.number}>{index + 1}</div>
                <div className={styles.image}><img src={topseller.image || defaultimage} className={styles.itemimage} alt={topseller.name} /></div>
                <div className={styles.details}>
                  <div className={styles.name}>{topseller.full_name}</div>
                  <div className={styles.ordered}>{topseller.total_items_sold} Sold</div>
                </div>
              </div>
            ))}
          </div>
