  ...
  useEffect(() => {
    const fetchTopSellers = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/home/topseller/${timeFrame.toLowerCase()}`, {
          method: 'GET',
          headers: {
    ...
    fetchTopSellers();
}, [timeFrame2]); // This will re-fetch when timeFrame changes
        ...
        <div className={styles.topitem2}>
          ...
          <div className={styles.contentcontainer}>
            {items.map((item, index) => (
              <div key={index} className={styles.content}>
                <div className={styles.number}>{index + 1}</div>
                <div className={styles.image}><img src={topSellers.image || defaultimage} className={styles.itemimage} alt={item.name} /></div>
                <div className={styles.details}>
                  <div className={styles.name}>{topSellers.full_name}HELLo</div>
                  <div className={styles.rating}>
                    {renderStars(item.avg_star_rating)}
                  </div>
