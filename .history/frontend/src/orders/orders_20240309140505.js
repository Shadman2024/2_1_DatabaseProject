  ...
  return (
    ...
    <div className={styles.orders_container}>
      ...
      {Object.entries(ordersByStatus).map(([status, orders]) => (
        <div key={status} className={styles.title}>
              ...
              {order.status === "delivered" && (
                order.items.map((item, index) => (
                  <div key={index} className={styles.reviewSection}>
                    ...
                    <input type="text" placeholder="Write a review..." onChange={(e) => item.reviewContent = e.target.value} />
                    <select onChange={(e) => item.starRating = parseInt(e.target.value, 10)} defaultValue={0}>
                      <option value={0} disabled>Rate...</option>
                      {[1, 2, 3, 4, 5].map(star => <option key={star} value={star}>{renderStars(star)}</option>)}
                    </select>
                    <button onClick={() => submitReview(item.id, item.reviewContent, item.starRating)}>Submit Review</button>
                  </div>
                ))
