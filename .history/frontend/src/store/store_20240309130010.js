...
const OrdersForAllItems = ({ setAuth }) => {
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        try {
            const response = await fetch(`http://localhost:5000/set/${orderId}/${newStatus}`, {
                method: 'PUT',
                headers: {
                    ...
                    {orders.map((order) => (
                        <div key={order.order_id} className={styles.order}>
                            ...
                            {(status === 'placed' || status === 'confirmed') && (
                                <Fragment>
                                    <button onClick={() => updateOrderStatus(order.order_id, 'confirmed')} className={styles.animated_button} style={{ marginRight: '10px' }}>Confirm</button>
