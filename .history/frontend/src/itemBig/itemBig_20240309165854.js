...
import { faStar, faDollarSign, faCircleChevronRight, faCircleChevronLeft ,faThumbsUp} from '@fortawesome/free-solid-svg-icons';
...
const ItemBig = () => {
    ...
    const handleVote = async (reviewId, voteType) => {
        try {
            const response = await fetch(`http://localhost:5000/reviews/vote`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'token': localStorage.token,
            ...
            // Optionally refresh the item details to reflect the new vote counts
            // fetchItemDetailsAndReviews();
            console.log('Vote registered successfully');
    
            // Refresh the reviews to show the updated vote counts
            // You might want to modify your backend to return the updated review or fetch updated reviews again.
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    <div key={index} className={styles.review}>
        ...
        <div className={styles.voteButtons}>
            <button onClick={() => handleVote(review.review_id, 'upvote')}><FontAwesomeIcon icon={faThumbsUp} size="2xl" /></button>
            <button onClick={() => handleVote(review.review_id, 'downvote')}>Downvote</button>
        </div>
        <div>Upvotes: {review.upvotes}, Downvotes: {review.downvotes}</div>
    </div>
