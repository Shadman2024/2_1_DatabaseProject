...
router.get('/home/topsellers/:timeFrame', async (req, res) => {
    ...
    // Assuming the week starts on Sunday (0) and ends on Saturday (6)
const now = new Date();
...
const endDate = lastDayOfWeek.toISOString().slice(0, 10);
    ...
    WITH sales AS (
        ...
        JOIN items it ON od.item_id = it.item_id
        JOIN users u ON it.user_id = u.user_id
        WHERE 
            o.created_at::date >= '2023-01-01' AND o.created_at::date <= '2023-01-07' -- Example week
        GROUP BY 
